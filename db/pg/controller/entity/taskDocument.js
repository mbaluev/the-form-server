...
const list = async (client, data) => {
  try {
    ...
    if (taskId) {
      const query1 = `SELECT id, taskid, title FROM taskdocuments WHERE taskid = $1`
      const params1 = [taskId];
      const res1 = await client.query(query1, params1);
      return res1.rows.map(mapRow);
    }
    const query1 = `SELECT id, taskid, title FROM taskdocuments`
...
const get = async (client, data) => {
  try {
    const id = data.id;
    const query1 = `SELECT id, taskid, title FROM taskdocuments WHERE id = $1`
    const params1 = [id]
    const res1 = await client.query(query1, params1);
...
const create = async (client, data) => {
  try {
    const query1 = 'INSERT INTO taskdocuments (id, taskid, title) VALUES ($1,$2,$3)';
...
const update = async (client, data) => {
  try {
    const query1 = `UPDATE taskdocuments SET 
...
const del = async (client, id) => {
  try {
    const query1 = `DELETE FROM taskdocuments WHERE id = $1`;
    const params1 = [id];
    await client.query(query1, params1);
    return true;
  } catch (err) {
